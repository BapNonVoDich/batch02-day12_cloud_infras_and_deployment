...
def load_users() -> List[Dict[str, Any]]:
    try:
        content = r.get("db:users")
        if content:
            users = json.loads(content)
            return [get_user_with_targets(u) for u in users]
        
        # Thử đọc từ mock data ban đầu nếu Redis trống
        ...
        if os.path.exists(USERS_FILE):
            with open(USERS_FILE, "r", encoding="utf-8") as f:
                ...
                if content:
                    users = json.loads(content)
                    save_users(users) # Lưu lại vào Redis
                    return [get_user_with_targets(u) for u in users]
    except Exception as e:
...
def save_users(users: List[Dict[str, Any]]):
    try:
        ...
        r.set("db:users", json.dumps(users_to_save, ensure_ascii=False))
    except Exception as e:
        print(f"Error saving users: {e}")

def load_chat_history(user_id: str) -> List[Dict[str, Any]]:
    try:
        content = r.get(f"db:chat_history:{user_id}")
        if content:
...
def save_chat_history(user_id: str, messages: List[Dict[str, Any]]):
    try:
        r.set(f"db:chat_history:{user_id}", json.dumps(messages, ensure_ascii=False))
    except Exception as e:
